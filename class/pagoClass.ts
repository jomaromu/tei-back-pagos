...
export class PagoClass {
  ...
  obtenerPagoID(req: any, resp: Response): void {
    ...
    pedidoModel.findById(
      pedido,
      (err: CallbackError, pedidoDB: PedidoModelInterface) => {
        if (err) {
          return resp.json({
            ok: false,
            mensaje: `Error interno`,
      ...
      (err: CallbackError, pedidoDB: PedidoModelInterface) => {
        ...
        pagosModel.findById(id, (err: CallbackError, pagoDB: PagoInterface) => {
          ...
          pagosModel.findByIdAndUpdate(
            ...
            { new: true },
            (err: CallbackError, pagoActualizadoDB: any) => {
              if (err) {
                return resp.json({
                  ok: false,
                  mensaje: `Error interno`,
              ...
              pedidoModel
                .findByIdAndUpdate(pedido, { total: total }, { new: true })
                .exec(async (err: CallbackError, pedidoActualizadoDB: any) => {
                  if (err) {
                    return resp.json({
                      ok: false,
  ...
  desactivarPago(req: any, resp: Response): void {
    ...
    pedidoModel.findById(pedido).exec((err: any, pedidoDB: any) => {
      if (err) {
        return resp.json({
          ok: false,
          mensaje: `Error interno`,
      ...
      pagosModel
        .findByIdAndUpdate(idPago, { motivo, estado: false }, { new: true })
        .exec((err: any, pagoDB: any) => {
          ...
          pedidoModel
            .findByIdAndUpdate(pedido, {}, { new: true })
            .populate({ path: "pagos_pedido", populate: "idCreador metodo" })
            .populate("productos_pedidos")
            .populate("sucursal")
            .exec(async (err: CallbackError, pedidoDB: any) => {
